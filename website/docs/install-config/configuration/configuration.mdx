...
These settings affect where files are stored. Files include application and source icons (media), as well as policy and blueprint attachments (reports). By default, files are stored on disk in the `/data` directory of the authentik container. S3-compatible storage is also supported.

### Backend Configuration

- `AUTHENTIK_STORAGE__BACKEND`: Storage backend to use. Valid values are `file` and `s3`. Defaults to `file`.

### File Storage (Default)

When using `file` backend, files are stored in `/data/` in the container with automatic path prefixes based on file type and tenant. No additional configuration is required.

### S3-Compatible Storage

When using the `s3` backend, all files are stored in the same bucket with automatic key prefixes based on file type and tenant.

#### Authentication

- `AUTHENTIK_STORAGE__S3__SESSION_PROFILE`: AWS profile to use for authentication. If set, uses credentials from `~/.aws/credentials`. Supports hot-reloading.
- `AUTHENTIK_STORAGE__S3__ACCESS_KEY`: S3 access key ID for authentication. Required if not using `SESSION_PROFILE`. Supports hot-reloading.
- `AUTHENTIK_STORAGE__S3__SECRET_KEY`: S3 secret access key for authentication. Required if not using `SESSION_PROFILE`. Supports hot-reloading.
- `AUTHENTIK_STORAGE__S3__SECURITY_TOKEN`: S3 session token for temporary credentials (e.g., when using IAM roles). Optional. Supports hot-reloading.

#### Connection Settings

- `AUTHENTIK_STORAGE__S3__BUCKET_NAME`: Name of the S3 bucket to store files in. Required.
- `AUTHENTIK_STORAGE__S3__REGION`: AWS region for the bucket (e.g., `us-east-1`).
- `AUTHENTIK_STORAGE__S3__ENDPOINT`: Custom endpoint URL for S3-compatible storage providers. Must be a full URL (e.g., `https://account-id.r2.cloudflarestorage.com`). Leave unset for AWS S3.
- `AUTHENTIK_STORAGE__S3__ADDRESSING_STYLE`: S3 URL addressing style. Valid values are `auto`, `path`, or `virtual`. Defaults to `auto`.

#### URL Generation

- `AUTHENTIK_STORAGE__S3__SECURE_URLS`: Whether generated URLs use HTTPS. Defaults to `true`.
- `AUTHENTIK_STORAGE__S3__CUSTOM_DOMAIN`: Custom domain for accessing files. Should be just the hostname (e.g., `cdn.example.com`), without the bucket name or path. Used to replace the S3 endpoint hostname in generated URLs. Optional.
- `AUTHENTIK_STORAGE__S3__PRESIGNED_EXPIRY`: How long presigned URLs remain valid, in seconds. Defaults to `3600` (1 hour).
